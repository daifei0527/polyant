...
配置可通过环境变量覆盖，前缀为 `POLYANT_`：

```bash
# 节点配置
export POLYANT_NODE_TYPE=seed
export POLYANT_NODE_NAME=my-node
export POLYANT_NODE_DATA_DIR=/data/polyant
...
export POLYANT_SYNC_INTERVAL_SECONDS=300

# 存储配置
export POLYANT_STORAGE_KV_TYPE=pebble
export POLYANT_STORAGE_SEARCH_TYPE=bleve
```

环境变量命名规则：`POLYANT_<节>_<字段>`，使用下划线连接。例如：
- `POLYANT_NODE_TYPE` → `node.type`
- `POLYANT_NETWORK_API_PORT` → `network.api_port`
...
创建服务文件 `/etc/systemd/system/polyant.service`：
...
```bash
# 创建用户
sudo useradd -r -s /bin/false polyant

# 创建目录
sudo mkdir -p /opt/polyant/{data,logs,keys}
sudo chown -R polyant:polyant /opt/polyant

# 安装服务
sudo cp polyant.service /etc/systemd/system/
sudo systemctl daemon-reload
sudo systemctl enable polyant
sudo systemctl start polyant

# 查看状态
sudo systemctl status polyant

# 查看日志
sudo journalctl -u polyant -f
```
...
1. **创建种子节点配置** `/opt/polyant/config.json`：
...
```json
{
  "node": {
    "type": "seed",
    "name": "seed-node-1",
    "data_dir": "/opt/polyant/data",
    "log_dir": "/opt/polyant/logs",
    "log_level": "info"
  },
  "network": {
    "listen_port": 9000,
...
```bash
sudo useradd -r -s /bin/false polyant
sudo mkdir -p /opt/polyant/{data,logs,keys,seed-data}
sudo chown -R polyant:polyant /opt/polyant
```
...
cp -r /path/to/seed-data/* /opt/polyant/seed-data/
chown -R polyant:polyant /opt/polyant/seed-data
```
...
```bash
# 检查服务状态
systemctl status polyant

# 查看实时日志
journalctl -u polyant -f

# 检查端口监听
netstat -tlnp | grep polyant
...
选项:
  ...
  --key-dir <dir>        密钥目录 (默认: ~/.polyant/keys)
```
...
```bash
# 停止服务
systemctl stop polyant

# 备份数据目录
tar -czf polyant-backup-$(date +%Y%m%d-%H%M%S).tar.gz ~/.polyant/data

# 备份密钥（重要！）
cp -r ~/.polyant/keys ~/.polyant/keys-backup

# 重启服务
systemctl start polyant
```
...
```bash
# 使用 rsync 增量同步
rsync -av --delete ~/.polyant/data/ /backup/polyant-data/
```

#### 方式三：定时备份脚本

创建 `/etc/cron.daily/polyant-backup`：

```bash
#!/bin/bash
BACKUP_DIR="/backup/polyant"
DATE=$(date +%Y%m%d)
RETENTION_DAYS=7

# 创建备份
tar -czf ${BACKUP_DIR}/polyant-${DATE}.tar.gz ~/.polyant/data

# 清理旧备份
find ${BACKUP_DIR} -name "polyant-*.tar.gz" -mtime +${RETENTION_DAYS} -delete
```
...
# 恢复数据
tar -xzf polyant-backup-20240101.tar.gz -C ~/

# 确保权限正确
chown -R polyant:polyant ~/.polyant
...
1. **源服务器**：
   ...
   # 打包数据
   tar -czf polyant-migration.tar.gz ~/.polyant/data ~/.polyant/keys
   ```

2. **传输数据**：
   ```bash
   scp polyant-migration.tar.gz user@new-server:/tmp/
   ```
...
# 删除旧索引
rm -rf ~/.polyant/data/search.bleve
...
```bash
# 实时查看日志
tail -f ~/.polyant/logs/polyant.log
...
1. **密钥保护**: 私钥文件权限设置为 600
   ```bash
   chmod 600 ~/.polyant/keys/private.key
   ```
...
```bash
# 检查是否有进程占用
lsof ~/.polyant/data/kv

# 终止旧进程
pkill -f polyant

# 或清理锁文件
rm -f ~/.polyant/data/kv/LOCK
```
...
# 清理日志
find ~/.polyant/logs -name "*.log" -mtime +30 -delete
...
```bash
# 查看同步相关日志
grep -i sync ~/.polyant/logs/polyant.log
```
...
# 搜索错误日志
grep -i error ~/.polyant/logs/polyant.log

# 查看最近 100 行
tail -100 ~/.polyant/logs/polyant.log

# 按时间过滤
journalctl -u polyant --since "1 hour ago"
```
...
# 方式二：环境变量
export POLYANT_NODE_LOG_LEVEL=debug
```
