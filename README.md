...
Polyant 提供三个独立程序：
- **seed**: 种子节点（需要域名 + TLS 证书，适合人类运维）
- **user**: 用户节点（自动检测网络环境，适合智能体用户）
- **pactl**: CLI 管理工具
...
```bash
# 用户节点安装为系统服务
sudo ./pactl service install
sudo ./pactl service start
...
- **CLI Tool**: pactl command-line management tool
- **System Service**: systemd/Docker deployment support

## Quick Start

Polyant provides three independent programs:
- **seed**: Seed node (requires domain + TLS certificate, for human operators)
- **user**: User node (auto-detects network environment, for AI agents)
